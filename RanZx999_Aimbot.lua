...
local Connections = {}
local ESPObjects = {}
local LockedAimPart = "Head"

--// CUSTOM SMOOTH FOV CIRCLE WITH THICK LINES
local FOVCircle = {
    Visible = false,
    Color = Color3.fromRGB(255, 255, 255),
    Radius = Config.Aimlock.FOVSize,
    Segments = {}
}

local function createSmoothCircle()
    for _, segment in pairs(FOVCircle.Segments) do
        pcall(function() segment:Remove() end)
    end
    FOVCircle.Segments = {}
...
local function updateFOVCircle()
    ...
    local numSegments = #FOVCircle.Segments
    
    for i = 1, numSegments do
        local angle1 = (i - 1) * (math.pi * 2 / numSegments)
        local angle2 = i * (math.pi * 2 / numSegments)
        
        local point1 = Vector2.new(
            center.X + math.cos(angle1) * FOVCircle.Radius,
            center.Y + math.sin(angle1) * FOVCircle.Radius
        )
        local point2 = Vector2.new(
            center.X + math.cos(angle2) * FOVCircle.Radius,
            center.Y + math.sin(angle2) * FOVCircle.Radius
        )
        
        local segment = FOVCircle.Segments[i]
        segment.From = point1
        segment.To = point2
        segment.Color = FOVCircle.Color
        ...
        if player ~= LocalPlayer and isAlive(player) and player.Character then
            ...
            if head then
                ...
                if onScreen and isVisible(head, player) then
                    ...
                    if Config.Aimlock.FOVEnabled and distance > Config.Aimlock.FOVSize then
                        continue
                    end
...
local function StartAimlock()
    ...
    AimlockConnection = RunService.RenderStepped:Connect(function()
        ...
        if Config.Aimlock.FOVEnabled then
            FOVCircle.Visible = true
            FOVCircle.Radius = Config.Aimlock.FOVSize
            updateFOVCircle()
        else
            FOVCircle.Visible = false
            updateFOVCircle()
        ...
        if Config.Aimlock.AutoAim then
            ...
            if newTarget ~= CurrentTarget then
                ...
                if Config.Aimlock.AimPart == "Random" then
                    local parts = {"Head", "UpperTorso", "LowerTorso"}
                    LockedAimPart = parts[math.random(1, #parts)]
                end
    ...
    for player, esp in pairs(ESPObjects) do
        ...
        if char and hrp and hum and hum.Health > 0 then
            ...
            if onScreen and dist <= Config.ESP.MaxDistance then
                local color = isTeammate(player) and Colors.Green or Colors.Primary
...
local function clearAllHighlights()
    for _, player in pairs(Players:GetPlayers()) do
        if player.Character then
            ...
            if hl and hl.Name == "RanZxHL" then
                hl:Destroy()
            end
        end
    end
...
local function updateHighlights()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        ...
        if existingHL and existingHL.Name == "RanZxHL" then 
            existingHL:Destroy() 
        end
        ...
        if Config.Highlight.Enabled then
            ...
            hl.Name = "RanZxHL"
            hl.Parent = char
            hl.FillColor = isTeammate(player) and Colors.Green or Colors.Primary
...
function CreateAimbotTab()
    ...
    CreateToggle("⭕ Show FOV Circle", Config.Aimlock.FOVEnabled, function(val)
        Config.Aimlock.FOVEnabled = val
        if not val then FOVCircle.Visible = false updateFOVCircle() end
    end)
    
    CreateSlider("FOV Size", 50, 500, Config.Aimlock.FOVSize, function(val)
        Config.Aimlock.FOVSize = val
    end)
...
function CreateESPTab()
    ...
    CreateToggle("✨ Highlight ESP", Config.Highlight.Enabled, function(val) 
        Config.Highlight.Enabled = val 
        if not val then clearAllHighlights() end
    end)
